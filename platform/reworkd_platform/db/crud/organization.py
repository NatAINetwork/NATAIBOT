...
from reworkd_platform.db.models.auth import Organization, organization_user
...
class OrganizationCrud(BaseCrud):
    ...
    async def get_by_name(self, name: str) -> Optional[OrganizationUsers]:
        owner = aliased(organization_user, name="owner")
        ...
        query = (
            ...
            .join(
                organization_user,
                and_(
                    Organization.id == organization_user.organization_id,
                ),
            )
            .join(
                User,
                User.id == organization_user.user_id,
            )
            .join(  # Owner
                owner,
                and_(
                    organization_user.organization_id == Organization.id,
                    organization_user.user_id == self.user.id,
                ),
            )
